...
def _emit_action_gateway_dry_run(*, target: str, message: str, platform_name: str) -> str:
    """Queue a Hermes send intent in FCAR Action Gateway dry-run mode.

    This is an explicit opt-in safety diversion. When enabled, send_message does
    not resolve platform credentials and does not call any platform adapter.
    """
    ...
    if not idempotency_key:
        ...
        idempotency_key = "hermes:send_message:" + hashlib.sha256(digest_source.encode("utf-8")).hexdigest()[:24]

    if not script.exists():
        return json.dumps(
            {
                "error": f"FCAR Action Gateway CLI not found: {script}",
                "action_gateway_dry_run": True,
                "sent": False,
            }
        )
    ...
    payload = {
        ...
        "source_tool": "hermes.send_message",
        "dry_run_only": True,
    }
    cmd = [
        sys.executable,
    ...
    try:
        ...
        return json.dumps(
            {
                "error": f"FCAR Action Gateway dry-run emit failed: {exc}",
                "action_gateway_dry_run": True,
                "sent": False,
            }
        )
...
def _handle_send(args):
    ...
    dry_run_parts = target.split(":", 1)
    dry_run_platform_name = dry_run_parts[0].strip().lower()
    if _action_gateway_dry_run_enabled():
        return _emit_action_gateway_dry_run(
            target=target,
            message=message,
            platform_name=dry_run_platform_name,
        )

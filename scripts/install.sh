...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        local npm_log
        npm_log="$(mktemp)"
        # The repository root is now an npm workspace. This stage installs
        # only root browser-tool dependencies; workspace applications have
        # their own installer stages below.
        if ! run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent --workspaces=false \
                >"$npm_log" 2>&1; then
